    ...
    if msg.from_user.id != sudo_id and msg.from_user.id != int(777000):
        ...
        if r.get(f"{sudo_id}welcome"):
            if not r.sismember(f"{sudo_id}accept", msg.chat.id):
                ...
                PM_logo = r.get(f"{sudo_id}:PMG_MEDIA") or "https://telegra.ph/file/efaec5a63fabeeb0eb5d1.jpg"
                CHANNEL = r.get(f"{sudo_id}:CHANNEL") or "K_O_O0"
                ...
                try:
                    await send(
                        msg.chat.id,
                        PM_logo,
                        caption=f"<b>{PM_text}</b>\n\n💠 [𝐌𝐘 𝐂𝐇𝐀𝐍𝐍𝐄𝐋](https://t.me/{CHANNEL}) | [𝐓𝐄𝐓𝐎](https://t.me/K_O_O0) 💠"
                    )
