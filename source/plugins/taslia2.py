...
async def permalink_onotha(c, msg):
    ...
    if not msg.reply_to_message:
        return
    if msg.reply_to_message.from_user.id == 6335965329:
        return await msg.edit(f"**- انت فاكره ابوك ارجل منك ده**")
    if msg.reply_to_message.from_user.id == 929431022:
        return await msg.edit(f"**- لكك دي هذا المطور قمورة وعلى راسك**")
    if msg.reply_to_message.from_user.id == 6335965329:
    ...
    animation_chars = [                "**🝳︙ جـاري الاتصـال بجهـاز الضحـية لأختـراقـة  📳**",     
       ...
       "**🝳︙ تحـميل الاخـتراق  ㊙️ .. 36%**\n█████████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒ `",        
        ...
        "**🝳︙ تحـميل الاخـتراق  ㊙️ .. 100%**\n████████████████████████`",       
         "**🝳︙ تـم اخـتراق الضحـية 🆘 بواسطه : `{هكرر تيتو}` . بـدون تنـازل قم بدفع مبلغ والا سيقوم بنشر صورك**",
            ]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
